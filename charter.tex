...
El Product Backlog se organiza en cuatro \'{e}picas, cada una con dos historias de usuario. El criterio para la asignación de Story Points fue la escala Fibonacci (1-2-3-5-8-13). Por lo tanto, solo se podrán asignar esos valores. Se pondera la complejidad técnica, el nivel de incertidumbre, por ejemplo, la disponibilidad de un dataset acotado para HU6; y la experiencia previa del alumno.
...
\begin{itemize}
  ...
  \item \textbf{\'{E}pica 3 --- Transfer learning entre modelos}
    \begin{itemize}
      \item \textbf{HU5} (Prioridad: Alta, 13 SP): Como responsable del proyecto, quiero entrenar un modelo sobre el dataset Amazon Reviews'23 para contar con una base amplia de productos y perfiles de usuario. Este modelo oficia como ``pre entrenado'' respecto al modelo dedicado
  ...
  \item \textbf{\'{E}pica 4 --- Exposición vía API y prueba de concepto}
    \begin{itemize}
      \item \textbf{HU7} (Prioridad: Alta, 8 SP): Como agente conversacional, quiero consultar \texttt{get\_profile(user)} y \texttt{recommend(context)} a través de un contrato de API estable para poder recomendar productos sin conocer el detalle interno del motor.
      \item \textbf{HU8} (Prioridad: Media, 8 SP): Como usuario final, quiero recibir recomendaciones de productos dentro de una conversación de compra simulada. Esto se hará mediante una prueba de concepto con una API vía Claude para validar que el sistema funciona de punta a punta.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{\'{E}pica 2}
    \begin{itemize}
      ...
      \item \textbf{HU4}: (a) cada recomendación devuelta incluye un score de confianza numérico en un rango definido. Un buen rango es $[0,1]$ ya que está normalizado y facilita comparar recomendaciones entre sí; (b) el score decrece de forma consistente ante menor evidencia disponible, como sería en el caso de un usuario nuevo; (c) existe un umbral configurable por debajo del cual la recomendación puede omitirse. El criterio de éxito será que aplicar el umbral de confianza reduzca la proporción de recomendaciones de baja relevancia sin descartar más del 20\% de las recomendaciones totales.
    \end{itemize}
  \item \textbf{\'{E}pica 3}
    \begin{itemize}
  ...
  \item \textbf{\'{E}pica 4}
    \begin{itemize}
      \item \textbf{HU7}: (a) \texttt{get\_profile(user)} responde en un formato de contrato documentado; (b) \texttt{recommend(context)} responde un ranking en formato documentado dentro de un tiempo inferior a 2 segundos en la prueba de concepto; (c) ambos contratos se mantienen estables ante cambios internos del motor, versionándose si cambian. El criterio de éxito será que el 100\% de las pruebas realizadas completen el ciclo \texttt{get\_profile} → \texttt{recommend} sin errores y dentro del tiempo de respuesta acotado.
...
\begin{enumerate}
  ...
  \item \textbf{Modelado:} el modelado se aborda como un problema híbrido, combinando perfilado mediante segmentación RFM y recomendación mediante enfoques colaborativo, de contenido y de co-compra. Además, se aplica transfer learning entre un modelo pre entrenado sobre un dataset extenso y un modelo dedicado ajustado sobre un dataset acotado.
  \item \textbf{Evaluación del modelo:} se mide la calidad de los rankings de recomendación y de la segmentación RFM, comparando el modelo dedicado contra una versión entrenada desde cero para validar la ganancia del transfer learning.
  \item \textbf{Despliegue del modelo (opcional):} para esta prueba de concepto, el despliegue se limita a exponer el motor vía una API simple consumida por un agente conversacional de prueba (API sobre Claude), sin publicación en infraestructura productiva.
\end{enumerate}
...
HU4 & Definir y calibrar un umbral configurable por debajo del cual la recomendación se omite o se reemplaza por un fallback & 4 h & Media \\ \hline
