...
Diseñar e implementar un sistema de perfilado y recomendación para retail, basado en un enfoque de transfer learning, que pueda integrarse como herramienta de un agente conversacional de compras. Busco que el sistema perfile a los usuarios a partir de features RFM y de comportamiento, y que genere recomendaciones híbridas de productos con una calidad aceptable incluso contando con pocos datos propios del cliente que lo adopte, aprovechando el conocimiento transferido desde un modelo pre entrenado sobre un dataset público y extenso. Esto es una prueba de concepto con la cual pretendo demostrar que es posible desacoplar la inteligencia de recomendación del canal conversacional específico. Este enfoque habilita su reutilización entre distintos bots y verticales de comercio. De esta manera, espero avanzar en mi propio desarrollo profesional dentro del área de inteligencia artificial conversacional.
...
\begin{itemize}
	...
	\item Exposición de los resultados vía una API con contrato estable (\texttt{get\_profile}, \texttt{recommend}).
	\item Diseño del esquema de persistencia en Postgres (reviews, perfiles de usuario, ítems, co-compra).
	\item Una prueba de concepto (POC) de un agente conversacional simple (API sobre Claude) que consuma la API para demostrar la integración.
\end{itemize}

El presente proyecto no incluye:
\begin{itemize}
	\item El desarrollo del flujo conversacional completo ni de la interfaz de usuario final (app o bot productivo).
	\item La integración con un bot conversacional real de producción.
