...
\begin{itemize}
  \item \textbf{\'{E}pica 1}
    \begin{itemize}
      \item \textbf{HU1}: (a) para todo usuario con al menos una reseña, se calculan recencia, frecuencia y valor monetario sin valores nulos; (b) los tres valores se normalizan en una escala común y el cálculo es reproducible ante los mismos datos de entrada; (c) el usuario queda asignado a un segmento RFM discreto y documentado (por ejemplo, ``champion'', ``en riesgo'', ``nuevo''). \textit{Criterio de éxito:} al menos el 95\% de los usuarios con reseñas registradas queda correctamente segmentado, sin valores nulos ni indefinidos.
      \item \textbf{HU2}: (a) el perfil incluye al menos dos atributos de comportamiento adicionales a RFM; (b) dichos atributos no duplican información ya cubierta por RFM; (c) se documenta el criterio de selección y el rango de valores esperado de cada atributo. \textit{Criterio de éxito:} los atributos adicionales mejoran la separación entre segmentos (por ejemplo, mayor distancia o cohesión entre clusters) respecto de usar solo RFM.
    \end{itemize}
  \item \textbf{\'{E}pica 2}
    \begin{itemize}
      \item \textbf{HU3}: (a) dado un perfil de usuario, se devuelve un ranking de al menos N ítems combinando las tres señales; (b) cada ítem recomendado incluye la traza de qué señal(es) contribuyeron a su score; (c) el ranking no repite ítems ya comprados o reseñados por el usuario, salvo que se solicite explícitamente. \textit{Criterio de éxito:} el ranking híbrido obtiene una métrica de calidad (por ejemplo, precision@10 o NDCG@10) superior a la de cualquiera de las tres señales evaluadas de forma aislada.
      \item \textbf{HU4}: (a) cada recomendación devuelta incluye un score de confianza numérico en un rango definido (por ejemplo, 0-1); (b) el score decrece de forma consistente ante menor evidencia disponible (por ejemplo, usuario cold-start); (c) existe un umbral configurable por debajo del cual la recomendación puede omitirse. \textit{Criterio de éxito:} aplicar el umbral de confianza reduce la proporción de recomendaciones de baja relevancia sin descartar más del 20\% de las recomendaciones totales.
    \end{itemize}
  \item \textbf{\'{E}pica 3}
    \begin{itemize}
  ...
  \item \textbf{\'{E}pica 4}
    \begin{itemize}
      \item \textbf{HU7}: (a) \texttt{get\_profile(user)} responde en un formato de contrato documentado; (b) \texttt{recommend(context)} responde un ranking en formato documentado dentro de un tiempo de respuesta acotado (por ejemplo, menor a 2 segundos en la PoC); (c) ambos contratos se mantienen estables ante cambios internos del motor, versionándose si cambian. \textit{Criterio de éxito:} el 100\% de las pruebas realizadas completan el ciclo \texttt{get\_profile} → \texttt{recommend} sin errores y dentro del tiempo de respuesta acotado.
      \item \textbf{HU8}: (a) un usuario simulado recibe al menos una recomendación dentro de una conversación con el agente; (b) el flujo de punta a punta queda documentado con al menos un ejemplo registrado; (c) se documentan al menos dos casos límite (usuario nuevo, ítem sin metadata) y cómo los maneja la PoC. \textit{Criterio de éxito:} al menos una conversación simulada de punta a punta concluye con una recomendación mostrada al usuario, incluyendo el manejo correcto de los casos límite documentados.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Comprensión del negocio:} el objetivo es perfilar usuarios y generar recomendaciones híbridas relevantes dentro de un flujo conversacional de retail; el valor agregado de la IA es habilitar una experiencia de vendedor proactivo; las métricas de éxito son la calidad de las recomendaciones (por ejemplo, precision@k) y evidencia de que el modelo dedicado supera a uno entrenado desde cero con menos datos.
  \item \textbf{Comprensión de los datos:} el dataset principal es Amazon Reviews'23 (reseñas, calificaciones y metadata de productos); se releva su volumen, cobertura de metadata y el sesgo propio de basarse en reseñas, ya que solo se observa al usuario que dejó reseña.
  \item \textbf{Preparación de los datos:} construcción de las features RFM por usuario, filtrado o marcado de ítems con metadata incompleta, y diseño del esquema de persistencia en PostgreSQL (reviews, perfiles, ítems en JSONB, tabla de co-compra).
  \item \textbf{Modelado:} se aborda como un problema híbrido de perfilado (segmentación RFM) y recomendación (colaborativo + contenido + co-compra), aplicando transfer learning entre un modelo pre entrenado (dataset extenso) y un modelo dedicado (dataset acotado).
...
A partir de las historias de usuario definidas en la sección 6, se descompone el trabajo en tareas técnicas concretas y acotadas en el tiempo. El criterio de estimación considera la complejidad técnica, el nivel de incertidumbre (por ejemplo, la disponibilidad de un dataset acotado para el modelo dedicado) y la experiencia previa del alumno; la prioridad de cada tarea depende de su relevancia funcional y de si resulta bloqueante para otras tareas.
...
HU1 & Extraer y limpiar los campos necesarios por usuario (fecha de reseña, rating/monto, producto) & 4 h & Alta \\ \hline
...
HU2 & Explorar candidatos de atributos de comportamiento adicionales (categorías frecuentes, recencia por categoría, diversidad de compra) & 3 h & Media \\ \hline
...
HU2 & Validar que los nuevos atributos no dupliquen información ya cubierta por RFM (correlación, redundancia) & 3 h & Media \\ \hline
...
HU3 & Construir una señal de contenido, buscando ítems similares entre sí a partir de su metadata (categoría, atributos, descripción) & 8 h & Alta \\ \hline
...
HU4 & Definir qué señales de evidencia disponible se usan para estimar la confianza (señales activas, cobertura de datos, cold-start) & 3 h & Media \\ \hline
...
HU6 & Identificar o construir un dataset más acotado (menor volumen o riqueza de atributos) adecuado para el ajuste & 6 h & Alta \\ \hline
...
HU7 & Probar ambos endpoints de punta a punta (tiempos de respuesta, formatos) y documentar & 3 h & Alta \\ \hline
HU8 & Diseñar el flujo conversacional básico de la PoC (saludo, búsqueda, recomendación, cierre) & 3 h & Media \\ \hline
...
HU8 & Definir usuarios y escenarios de prueba simulados (usuario recurrente, usuario nuevo/cold-start, ítem sin metadata) & 3 h & Media \\ \hline
HU8 & Ejecutar y registrar al menos un ejemplo de conversación de punta a punta (log o transcripción) & 2 h & Media \\ \hline
HU8 & Documentar cómo la PoC maneja los casos límite identificados & 2 h & Media \\ \hline
