...
\begin{itemize}
  \item \textbf{\'{E}pica 1 --- Perfilado de usuarios}
    \begin{itemize}
      \item \textbf{HU1} (Prioridad: Alta, 8 SP): Como sistema de perfilado, quiero calcular las variables RFM (recencia, frecuencia, valor monetario) de cada usuario a partir de sus reseñas para poder clasificarlo según su comportamiento de consumo.
      \item \textbf{HU2} (Prioridad: Media, 5 SP): Como sistema de perfilado, quiero enriquecer el perfil RFM con atributos de comportamiento adicionales (categorías más frecuentes, recencia por categoría) para mejorar la calidad de la segmentación.
    \end{itemize}
  \item \textbf{\'{E}pica 2 --- Motor de recomendación híbrido}
    \begin{itemize}
      \item \textbf{HU3} (Prioridad: Alta, 13 SP): Como recomendador, quiero combinar señales colaborativas, de contenido y de co-compra en un único ranking de ítems para ofrecer recomendaciones más relevantes que un solo enfoque aislado.
      \item \textbf{HU4} (Prioridad: Media, 5 SP): Como recomendador, quiero exponer un score de confianza por recomendación para que el agente conversacional pueda decidir cuándo mostrarla u ofrecer alternativas.
    \end{itemize}
