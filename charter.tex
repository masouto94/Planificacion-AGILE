...
El Product Backlog se organiza en cuatro \'{e}picas, cada una con dos historias de usuario. El criterio de Story Points adoptado es una escala Fibonacci (1-2-3-5-8-13), ponderando la complejidad técnica, el nivel de incertidumbre (por ejemplo, la disponibilidad de un dataset acotado para HU6) y la experiencia previa del alumno.
...
\begin{itemize}
  \item \textbf{\'{E}pica 1}
    \begin{itemize}
      ...
      \item \textbf{HU2}: (a) el perfil incluye al menos dos atributos de comportamiento adicionales a RFM; (b) dichos atributos no duplican información ya cubierta por RFM; (c) se documenta el criterio de selección y el rango de valores esperado de cada atributo.
    \end{itemize}
  \item \textbf{\'{E}pica 2}
    \begin{itemize}
      ...
      \item \textbf{HU4}: (a) cada recomendación devuelta incluye un score de confianza numérico en un rango definido (por ejemplo, 0-1); (b) el score decrece de forma consistente ante menor evidencia disponible (por ejemplo, usuario cold-start); (c) existe un umbral configurable por debajo del cual la recomendación puede omitirse.
    \end{itemize}
  \item \textbf{\'{E}pica 3}
    \begin{itemize}
      \item \textbf{HU5}: (a) se documenta la muestra de Amazon Reviews'23 utilizada (tamaño, categorías, período); (b) se reporta al menos una métrica de calidad sobre un conjunto de validación; (c) el modelo y sus artefactos (pesos, features) quedan versionados y son reutilizables por el modelo dedicado.
      \item \textbf{HU6}: (a) se declara explícitamente el dataset acotado utilizado para el ajuste; (b) se compara al menos una métrica de calidad entre el modelo dedicado y una alternativa entrenada desde cero; (c) el proceso de ajuste es reproducible sobre el mismo dataset acotado.
    \end{itemize}
  \item \textbf{\'{E}pica 4}
    \begin{itemize}
      \item \textbf{HU7}: (a) \texttt{get\_profile(user)} responde en un formato de contrato documentado; (b) \texttt{recommend(context)} responde un ranking en formato documentado dentro de un tiempo de respuesta acotado (por ejemplo, menor a 2 segundos en la PoC); (c) ambos contratos se mantienen estables ante cambios internos del motor, versionándose si cambian.
      \item \textbf{HU8}: (a) un usuario simulado recibe al menos una recomendación dentro de una conversación con el agente; (b) el flujo de punta a punta queda documentado con al menos un ejemplo registrado; (c) se documentan al menos dos casos límite (usuario nuevo, ítem sin metadata) y cómo los maneja la PoC.
    \end{itemize}
\end{itemize}
